...
const INSTALLATION_ID: string = process.env.INSTALLATION_ID || "";

if (!APP_ID || !APP_PRIVATE_KEY || !INSTALLATION_ID) {
  throw new Error("Missing required environment variables: APP_ID, APP_PRIVATE_KEY, or INSTALLATION_ID");
}
...
async function getInstallationAccessToken(jwtToken: string, installationId: string): Promise<string> {
  const auth = createAppAuth({
...
async function main(): Promise<void> {
  try {
    ...
    const accessToken = await getInstallationAccessToken(jwtToken, INSTALLATION_ID);
