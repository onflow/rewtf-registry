...
class RegistryValidator {
	...
	private async validateRequiredFields(entry: any): Promise<void> {
		...
		if (
			...
			entry.github.length === 0
		) {
			this.errors.push(
				"Missing or invalid 'github' field - must be a non-empty array",
			);
		}
		...
		if (
			...
			this.errors.push(
				"Missing or invalid 'repos' field - must be a non-empty array",
			);
		}
		...
		for (const handle of handles) {
			...
			try {
				...
				if (response.status !== 200) {
					this.errors.push(
						`GitHub handle '${handle}' is not accessible (status: ${response.status})`,
					);
				}
			} catch (error) {
	...
	private async validateRepositoryUrls(urls: string[]): Promise<void> {
		for (const url of urls) {
			...
			try {
				...
				if (response.status !== 200) {
					this.errors.push(
						`Repository '${url}' is not accessible (status: ${response.status})`,
					);
				}
			} catch (error) {
	...
	private validateWalletAddresses(wallets: any): void {
		...
		if (!wallets.flow || typeof wallets.flow !== "string") {
			...
			if (
				!flowAddress.startsWith("0x") ||
				flowAddress.length !== 18 ||
				!/^0x[0-9a-fA-F]{16}$/.test(flowAddress)
			) {
				this.errors.push(
